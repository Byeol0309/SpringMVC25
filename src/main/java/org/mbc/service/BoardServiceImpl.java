...
@Service 			// 이 클래스는 서비스 계층임을 명시함.
@AllArgsConstructor // 생성자 자동주입 (new BoardServiceImpl(mapper))
@Log4j2 			// 로깅처리용
public class BoardServiceImpl implements BoardService{
	//조원이 실행문을 생성하는 용도로 활용 필수 : implements BoardService (구현체)
	...
	public BoardVO get(Long bno) {
		// 게시물의 번호가 들어가면 메퍼의 select문이 동작해야한다.
	...
	public boolean modify(BoardVO board) {
		// 프론트에서 객체가 넘어오면 메퍼를 통해서 update쿼리가 실행된다.
	...
	public boolean remove(Long bno) {
		// 프론트에서 번호가 넘어오면 메퍼를 통해서 delete 쿼리가 실행
	...
	public List<BoardVO> getList() {
		...
		log.info("BoardServiceImpl.getList메서드실행...............");
