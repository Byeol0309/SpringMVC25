...
@Controller 
@RequestMapping("/sample/*") //http://192.168.111.104:80/controller/sample/의 모든것에 반응한다
...
public class SampleController {
	...
	public void basic() {
		...
		log.info("리턴이 void이기때문에 basic.jsp파일을 찾습니다.");
	...
	public String ex01(SampleDTO dto) { //SampleDTO dto = new SampleDTO();
		...
		return "mbcex01"; //WEB-INF/views/mbcex01.jsp -> servlet-context.xml
	} //ex01()메서드 종료
	
	@GetMapping("/ex02")// http://192.168.111.104:80/controller/sample/ex02
	private String ex02(@RequestParam("owner") String name, 
			@RequestParam("age") int age ) {
			//프론트 변수와 백엔드의 변수가 다를때 (초보자는 일치시킬것)
		log.info("name : " + name );
		log.info("age : " + age );		
		return "ex02";
	}
	...
	@GetMapping("ex02List")
	//http://192.168.111.104:80/controller/sample/ex02List?ids=111&ids222&ids=333
	...
	public String ex02Bean(SampleDTOList list) {
		//url 경로에 배열로 들어오는파라미터
		// /ex02Bean?list[0].name=kkw&list[2].age=33
		// http://192.168.111.104/controller/sample/ex02Bean?list[0].name=kkw&list[2].age=33
	...
	public void ex05() {
		...
		//리턴이 void인 경우에는 자동으로 [WEB-INF/views/sample/ex05.jsp]를찾는다
		//초보자 사용 금지
	}
